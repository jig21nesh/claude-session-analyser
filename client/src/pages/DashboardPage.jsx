...
import RangeFilter, { sinceForPreset } from '../components/RangeFilter.jsx';
...
import { fillDailyGaps } from '../utils/series.js';
...
export default function DashboardPage() {
  ...
  const showForecast = range === 'all';

  const summary = useApi(() => api.summary(since), [since]);
  const daily = useApi(() => api.dailyCosts(null, since), [since]);
  const predictions = useApi(() => api.predictions(30), []);
  const projects = useApi(() => api.projects(since), [since]);
  ...
  return (
    <>
      ...
      <RangeFilter value={range} onChange={setRange} firstActivity={totals.first_activity} />

      {totals.sessions === 0 ? (
        <Empty message={range === 'all'
          ? 'No sessions analysed yet — the first scan may still be running. Hit Refresh in a few seconds.'
        ...
        <>
          ...
          <div className="card">
            <div className="card-title">
              {showForecast ? 'Daily cost & 30-day forecast' : 'Daily cost'}
            </div>
            <p className="card-subtitle">
              {showForecast ? (
                <>
                  Solid gold: actual spend per day. Dashed blue:{' '}
                  {predictions.data ? MODEL_LABELS[predictions.data.model] : 'forecast'} with 80% interval.{' '}
                  <Link to="/help">How the forecast works →</Link>
                </>
              ) : (
                'Actual spend per day in the selected range. Pick “All” to see the forecast.'
              )}
            </p>
            {daily.loading ? (
              <Loading label="Loading daily costs…" />
            ) : daily.error ? (
              <ErrorState message={daily.error} />
            ) : (
              <>
                <DailyCostChart
                  history={fillDailyGaps(daily.data.days.map((d) => ({ date: d.date, cost: d.cost })))}
                  forecast={showForecast ? predictions.data?.forecast ?? [] : []}
                />
                {showForecast && <ForecastDetails result={predictions.data} />}
              </>
            )}
          </div>
